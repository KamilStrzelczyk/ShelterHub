...
public class IsValidatePasswordUseCase {
    ...
    Boolean passwordIsValidate(String password) {
        ...
        return password.length() >= 8 && password.length() <= 12 && // sprawdza długość
                password.matches(".*\\d.*") && // czy ma liczbę
                password.matches(".*[a-z].*") && // czy ma małą literę
                password.matches(".*[A-Z].*") && // czy ma dużą literę
